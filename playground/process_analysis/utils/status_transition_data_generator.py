...
def _create_first_status_change():
    project_key = rand.choice(
        ["CORE", "PLAT", "BRAN", "SUPP", "MISC"], p=[0.4, 0.2, 0.2, 0.1, 0.1]
    )
    ...
    return StatusChange(
        issue_key, issue_type, created_date, Status.BACKLOG, next_status, changed_date
    )
...
def _create_next_status_change(current: StatusChange) -> StatusChange | None:
    ...
    match current.to_status:
        case Status.BACKLOG:
            ...
            changed_date = current.changed_date + timedelta(
                hours=rand.gumbel(24 * 50, 24 * 20)
            )
        case Status.TODO:
            next_status = rand.choice(
                [
        ...
        case Status.TEST:
            ...
            changed_date = current.changed_date + timedelta(
                hours=rand.gumbel(24 * 2, 24 * 1)
            )
        case Status.RELEASE:
            next_status = rand.choice(
                [None, Status.TODO, Status.TEST, Status.DONE], p=[0.02, 0.01, 0.1, 0.87]
            )
            changed_date = current.changed_date + timedelta(
                hours=rand.gumbel(24 * 3, 24 * 1)
            )
        case Status.DONE:
            next_status = rand.choice(
                [None, Status.TODO, Status.IN_PROGRESS], p=[0.85, 0.02, 0.13]
            )
            changed_date = current.changed_date + timedelta(
                hours=rand.gumbel(24 * 7, 24 * 3)
            )
